...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item $x_a[n]=\alpha^{|n|}$ ,$1>|\alpha|>0$
    \end{enumerate}
\end{enumerate}
\solution The $z$ transform of $x_b[n]$ is given by 
\begin{align}
    x_a[z] &= \sum_{n=-\infty}^{\infty}x_a[n]z^{-n}\\
    &=\sum_{n=-\infty}^{\infty} \alpha^{|n|}z^{-n}\\
    &=\sum_{n=-\infty}^{-1}\alpha^{-n}z^{-n}+\sum_{n=0}^{\infty}\alpha^{n}z^{-n}\\
    &=\frac{\alpha z}{1-\alpha z}+\frac{1}{1-\frac{\alpha}{z}}\\
    &=\dfrac{z(1-\alpha^2)}{(1-\alpha z)(z-\alpha)}
\end{align}
 The given sequence $x_a[n]$ is two sided exponential sequence\\
 from above equation it is clear that the z transform $x_a[z]$ converges for\\
 \begin{align}
 |\alpha|<|z|<|\frac{1}{|\alpha|}
 \end{align}
  so \textbf{the ROC is annular region $|\alpha|<|z|<\dfrac{1}{\alpha}$} \\\\
   where annular region is meant by region between two conc circles.
\end{document}
